...
void main(){
    ...
    for(int i=0;i<uLightCount;i++){
        ...
        if(uLightType[i]==0){
            ...
            occlusion *= power * attn;

            float rampThresh = 0.1;
            float rampSmooth = 0.0;
            float ramp = smoothstep(rampThresh-rampSmooth*0.5,rampThresh+rampSmooth*0.5,occlusion);

            vec4 highlightColor = vec4(vec3(1.0),1.0);
            vec4 shadowColor = vec4(vec3(0.0),1.0);
            vec4 rampColor = lerp(shadowColor,highlightColor,ramp);
            
            diffuse += uLightColor[i]  * rampColor;
