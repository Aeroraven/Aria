...
export class AriaStageFog extends AriaStage{
    renderEnt:()=>void
    ...
    public async prepare(gl: WebGL2RenderingContext): Promise<any> {
        ...
        //Fog
        const plainFog = (<AriaComFog>AriaComFog.create(gl))
            .setFogColor(1,1,1)
            .setFogDist(5.1,31.3)
        ...
        const floorPbrMaterial = (<AriaComSimplePBR>AriaComSimplePBR.create(gl))
            .setAlbedo(1.0,0.4,0.1)
            .setMetallic(1.0)
            .setRoughness(0.5)
            .setAO(1.0)

        const pbrTexMaterial = (<AriaComPBR>AriaComPBR.create(gl))
            .setAO(assets.getTexture("wood/ao"))
            .setAlbedo(assets.getTexture("wood/albedo"))
            .setMetallic(assets.getTexture("wood/metal"))
            .setRoughness(assets.getTexture("wood/rough"))
            .setNormal(assets.getTexture("wood/normal"))
        ...
        for(let i=0;i<15;i++){
            ...
            const boxMesh = (<AriaComMesh>AriaComMesh.create(gl))
                .setCamera(camera)
                .setShader(assets.getShader("plaincube-pbr-t"))
                .setDepthShader(assets.getShader("shadow-dir-depth"))
                .setBuffer(boxBuffer)
                .setTexture(AriaComMeshTextureType.acmtDiffuse,boxTexture)
                .setLight(pointLight)
                .setMaterial(pbrTexMaterial)
                .addAttachments("fog",plainFog)
