...
import { AriaComSphere } from "../components/geometry/aria-com-sphere";
...
export class AriaStageSphere extends AriaStage{
    renderEnt:()=>void
    ...
    public async prepare(gl: WebGL2RenderingContext): Promise<any> {
        ...
        const fpGeometry = (<AriaComSphere>AriaComSphere.create(gl))
            .scale(0.5)
            .translate(0,0,0)
            .setDensity(40)
        ...
        const fpLight = (<AriaComLightSet>AriaComLightSet.create(gl))
            .addDirectionalLight([1,1,1],[10,10,10])
