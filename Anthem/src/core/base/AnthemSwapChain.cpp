...
namespace Anthem::Core{
    ...
    bool AnthemSwapChain::setSrgbPreference(bool preference) {
        this->preferSrgbColor = preference;
        return true;
    }
    ...
    bool AnthemSwapChain::specifySwapChainDetails(AnthemPhyDevice* phyDevice,const GLFWwindow* window){
        ...
        for(const auto& p:this->specifiedSwapChainDetails->formats){
            if (this->preferSrgbColor) {
                ...
                if (p.format == VK_FORMAT_B8G8R8A8_UNORM && p.colorSpace == VK_COLOR_SPACE_SRGB_NONLINEAR_KHR) {
                    this->specifiedSurfaceFormat = p;
                    fd = true;
                    break;
                }
