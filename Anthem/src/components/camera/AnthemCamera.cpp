...
namespace Anthem::Components::Camera{
    ...
    void AnthemCamera::calculateProjectionMatrix(AtMatf4& out){
        if(this->projType == AT_ACPT_PERSPECTIVE){
            out = AnthemLinAlg::spatialPerspectiveTransformWithFovAspect(this->frustumNearDepth,this->frustumFarDepth,this->frustumFov,this->frustumAspectRatio);
    ...
    void AnthemCamera::calculateViewMatrix(AtMatf4& out){
        ...
        out = AnthemLinAlg::modelLookAtTransform(this->pos,front,this->up);
    }
