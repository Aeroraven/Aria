...
namespace Anthem::Core{
    ...
    public:
        ...
        bool createColorAttachmentImage(AnthemImage** pImage, AnthemDescriptorPool* descPool, uint32_t bindLoc,
            AnthemImageFormat format, bool enableMsaa, uint32_t descId = -1, bool enableMipmap = false, int width = 0, int height = 0);
        ...
        bool drDrawIndexedIndirectSsbo(AnthemShaderStorageBuffer* buffer, uint32_t copy,uint32_t commands, uint32_t cmdIdx);
        ...
        //bool drForceSwapchainRepresentable(uint32_t swapchainImageIdx, uint32_t cmdIdx);
