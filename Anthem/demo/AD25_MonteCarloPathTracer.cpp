...
void initialize() {
	...
	st.cfg.demoName = "25. Monte Carlo Path Tracer";
	...
	st.camera.specifyPosition(0, -13, 5);
...
void loadModel() {
	...
	for (int i = 0; i < st.model.size(); i++) {
		...
		cv::Mat srcImg(texHeight, texWidth, CV_8UC4, texData);
		cv::Mat destImg;
		cv::resize(srcImg, destImg, { 1024,1024 });
		st.rd.createTexture(&st.image[i], st.descImage, destImg.data, 1024, 1024, texChannels, 0, false, false, AT_IF_SRGB_UINT8, -1, true);
