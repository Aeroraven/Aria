...
void prepareTarget(TargetPass& target,AnthemSimpleToyRenderer& renderer){
    ...
    renderer.createTexture(&target.image,target.descPool,texData,texWidth,texHeight,texChannels,0,true);
    ANTH_LOGI("Texture Created");

    //Depth Buffer
    renderer.createDepthBuffer(&target.depthBuffer);
...
void prepareOffscreen(OffscreenPass& offscreen,AnthemSimpleToyRenderer& renderer){
    ...
    for(auto chosenMesh=0;chosenMesh<gltfResult.size();chosenMesh++){
        ...
        renderer.createTexture(&offscreen.image[chosenMesh],offscreen.descPool[chosenMesh],texData,texWidth,texHeight,texChannels,1,true);
        offscreen.image[chosenMesh]->enableMipMapping();
    }
    ANTH_LOGI("Texture Created");

    //Create Depth Buffer
    renderer.createDepthBuffer(&offscreen.depthBuffer);
    ANTH_LOGI("Depth Buffer Created");

    //Create Pass
    AnthenRenderPassSetupOption setupOpt{
        .attachmentAccess = AT_ARP_INTERMEDIATE_PASS
    };
    renderer.setupRenderPass(&offscreen.pass,&setupOpt,offscreen.depthBuffer);
    ANTH_LOGI("Render Pass Created");

    //Create Attachment
    renderer.createColorAttachmentImage(&offscreen.colorAttachment, offscreen.descPoolColorAtt,0);
...
void recordCommandsTarget(AnthemConfig* cfg,AnthemSimpleToyRenderer& renderer, TargetPass& target, OffscreenPass& offscreen ,int i){
    renderer.drStartRenderPass(target.pass,(AnthemFramebuffer *)(target.framebuffer->getFramebufferObject(i)),i);
...
void recordCommandsOffscreen(AnthemConfig* cfg,AnthemSimpleToyRenderer& renderer, OffscreenPass& offscreen,int i){
    //Prepare Command
    renderer.drStartRenderPass(offscreen.pass,(AnthemFramebuffer *)(offscreen.framebuffer),i);
    renderer.drSetViewportScissor(i);
