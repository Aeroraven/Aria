...
struct Stage {
	...
	AnthemBottomLevelAccStruct* blasA = nullptr;
	AnthemBottomLevelAccStruct* blasB = nullptr;
	AnthemAccStructGeometry* blasGeoA = nullptr;
	AnthemAccStructGeometry* blasGeoB = nullptr;
	AnthemTopLevelAccStruct* tlas = nullptr;
	AnthemAccStructInstance* tlasInstA = nullptr;
	AnthemAccStructInstance* tlasInstB = nullptr;
...
void createAccelerationStruct() {
	...
	st.rd.createRayTracingGeometry(&st.blasGeoA, 3, vertexA, index, transform);
	st.rd.createRayTracingGeometry(&st.blasGeoB, 3, vertexB, index, transform);

	st.rd.createBottomLevelAS(&st.blasA);
	st.blasA->addGeometry({ st.blasGeoA });
	st.blasA->buildBLAS();

	st.rd.createBottomLevelAS(&st.blasB);
	st.blasB->addGeometry({ st.blasGeoB });
	st.blasB->buildBLAS();

	// Top-level AS
	st.rd.createRayTracingInstance(&st.tlasInstA, { st.blasB,st.blasA }, { transform,transform });
	st.rd.createTopLevelAS(&st.tlas);

	st.tlas->addInstance({ st.tlasInstA });
	st.tlas->buildTLAS();
...
void createPipeline() {
	...
	st.rd.createRayTracingPipeline(&st.pipeline, { dseAs,dseImg,dseUni }, {}, st.shader, 2);
}
